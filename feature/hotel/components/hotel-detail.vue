...
function isHotelMatchingUrl(): boolean {
  return (
    hotel.value?.data?.location?.continent.toLowerCase().replace(/\s+/g, '-') === params.continent
    && hotel.value?.data?.location?.country.toLowerCase().replace(/\s+/g, '-') === params.country
    && hotel.value?.data?.location?.city.toLowerCase().replace(/\s+/g, '-') === params.city
  )
}

const faqs = hotel.value.data?.faqs
    ...
    <div class="grid mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:grid-cols-12">
      <div class="mr-auto place-self-center lg:col-span-7">
        ...
        <p class="max-w-2xl mb-6 font-light text-gray-500 lg:mb-8 md:text-lg lg:text-xl dark:text-gray-400">
          {{ hotel.data.shortDescription }}
        </p>
      </div>
      <div class="hidden lg:mt-0 lg:col-span-5 lg:flex">
        <NuxtImg provider="contentful" :src="hotel.data.mainImage" alt="" width="600" height="600" />
      </div>
    </div>
    <div class="mb-10">
      ...
      <Location v-if="hotel.data.location" :is-rich-text-embedded="false" :data="hotel.data.location" />
    </div>
